...
```bash
sudo cp /srv/hse/deploy/hse-verify-payments.service /etc/systemd/system/
sudo cp /srv/hse/deploy/hse-verify-payments.timer   /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now hse-verify-payments.timer
```

هر ده دقیقه، تراکنش‌هایی را که پاسخ تأییدشان در شبکه گم شده دوباره از
زرین‌پال می‌پرسد. **بدون این تایمر، مشتری‌ای که پاسخ بانکش گم شده، پول
داده و دوره‌اش باز نمی‌شود** تا وقتی خودش تماس بگیرد. توضیح کامل در
README، بخش «وصل کردن درگاه زرین‌پال».
...
| مشتری پرداخت کرده ولی «در حال بررسی» مانده | پاسخ تأیید درگاه گم شده | `systemctl list-timers hse-verify-payments.timer` و `manage.py verify_pending_payments` |
| مبلغ کسرشده ده برابر یا یک‌دهم سفارش است | واحد پول | `ZARINPAL_CURRENCY` در `.env` (پیش‌فرض IRT = تومان) |
...
- [ ] **مبلغ کسرشده از کارت دقیقاً برابر مبلغ سفارش** (نه ده برابر، نه یک‌دهم)
- [ ] `hse-verify-payments.timer` فعال است
